...
class ReactionRepository(RedisRepository[models.Reaction]):
    ...
    async def create(
        ...
        await self._redis.set(
            name=reaction.key,
            value=msgpack.packb(
                dict(user_id=str(reaction.user_id), type=str(reaction.type.value))
            )
        )
    ...
    async def read_by_post_id(self, post_id: uuid.UUID) -> list[dto.Reaction]:
        ...
        reaction_keys = await self._redis.keys(REACTION_KEY_MASK.format(post_id=post_id, user_id="*"))
    ...
    async def delete(
        ...
        deleted = await self._redis.delete(REACTION_KEY_MASK.format(post_id=post_id, user_id=user_id))
        return bool(deleted)
